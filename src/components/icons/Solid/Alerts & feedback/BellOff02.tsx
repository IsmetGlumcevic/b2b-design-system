import * as React from "react";
import type { SVGProps } from "react";
const SvgBellOff02 = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    role="img"
    {...props}
  >
    <path
      fill="currentColor"
      fillRule="evenodd"
      d="M3.707 2.293a1 1 0 0 0-1.414 1.414l2.856 2.856Q5.001 7.27 5 8c0 2.913-.732 4.844-1.5 6.077l-.011.02c-.358.573-.64 1.027-.83 1.358a6 6 0 0 0-.247.467c-.032.07-.068.157-.096.251a1.2 1.2 0 0 0-.051.45c.01.136.039.37.173.611.134.242.318.39.428.47.25.186.538.229.634.243h.004c.147.023.315.034.484.04.335.013.799.013 1.376.013h11.222l3.707 3.707a1 1 0 0 0 1.414-1.414L6.997 5.583a1 1 0 0 0-.066-.066z"
      clipRule="evenodd"
    />
    <path
      fill="currentColor"
      d="M10 22a1 1 0 1 1 0-2h4a1 1 0 1 1 0 2zM21.53 17.288q.017-.026.032-.054c.134-.241.162-.475.173-.612a1.2 1.2 0 0 0-.05-.449 2 2 0 0 0-.097-.251 6 6 0 0 0-.247-.467c-.19-.33-.472-.784-.83-1.358l-.012-.02C19.732 12.844 19 10.913 19 8A7 7 0 0 0 7.173 2.93z"
    />
  </svg>
);
export default SvgBellOff02;
