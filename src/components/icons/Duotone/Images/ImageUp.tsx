import * as React from "react";
import type { SVGProps } from "react";
const SvgImageUp = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    role="img"
    {...props}
  >
    <g fill="currentColor" opacity={0.12}>
      <path d="M8.5 10.5a2 2 0 1 0 0-4 2 2 0 0 0 0 4M6.531 19.608l8.46-7.69c.358-.326.537-.489.742-.555a1 1 0 0 1 .557-.018c.208.053.398.205.776.508l2.797 2.238c.378.302.567.453.71.635a2 2 0 0 1 .374.778c.053.226.053.468.053.952 0 1.44 0 2.159-.242 2.724a3 3 0 0 1-1.578 1.578c-.565.242-1.285.242-2.724.242H7.07c-.643 0-.964 0-1.106-.125a.5.5 0 0 1-.166-.43c.02-.188.259-.404.734-.837" />
    </g>
    <path
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth={2}
      d="m16 5 3-3m0 0 3 3m-3-3v6m-6.5-5H7.8c-1.68 0-2.52 0-3.162.327a3 3 0 0 0-1.311 1.311C3 5.28 3 6.12 3 7.8v8.4c0 1.68 0 2.52.327 3.162a3 3 0 0 0 1.311 1.311C5.28 21 6.12 21 7.8 21H17c.93 0 1.395 0 1.776-.102a3 3 0 0 0 2.122-2.122C21 18.396 21 17.93 21 17M10.5 8.5a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4.49 3.418-8.459 7.69c-.476.433-.714.649-.735.836a.5.5 0 0 0 .167.431C6.105 21 6.426 21 7.07 21h9.387c1.44 0 2.159 0 2.724-.242a3 3 0 0 0 1.578-1.578c.242-.565.242-1.285.242-2.724 0-.484 0-.726-.053-.952a2 2 0 0 0-.374-.778c-.143-.182-.332-.333-.71-.636l-2.797-2.237c-.379-.303-.568-.454-.776-.508a1 1 0 0 0-.557.018c-.205.066-.384.23-.743.555"
    />
  </svg>
);
export default SvgImageUp;
