import * as React from "react";
import type { SVGProps } from "react";
const SvgServer03 = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    role="img"
    {...props}
  >
    <path
      fill="currentColor"
      d="M7.48 3h9.04c1.578 0 2.367 0 2.986.3a3 3 0 0 1 1.298 1.21c.341.598.396 1.385.504 2.96l.658 9.543A3.5 3.5 0 0 0 18.5 14h-13a3.5 3.5 0 0 0-3.466 3.013l.658-9.543c.108-1.575.163-2.362.504-2.96A3 3 0 0 1 4.494 3.3C5.113 3 5.902 3 7.48 3"
      opacity={0.12}
    />
    <path
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth={2}
      d="m22 17.5-.692-10.03c-.108-1.575-.163-2.362-.504-2.96a3 3 0 0 0-1.298-1.21C18.887 3 18.098 3 16.52 3H7.48c-1.578 0-2.367 0-2.986.3a3 3 0 0 0-1.298 1.21c-.341.598-.396 1.385-.504 2.96L2 17.5m20 0a3.5 3.5 0 0 1-3.5 3.5h-13A3.5 3.5 0 0 1 2 17.5m20 0a3.5 3.5 0 0 0-3.5-3.5h-13A3.5 3.5 0 0 0 2 17.5m4 0h.01m5.99 0h6"
    />
  </svg>
);
export default SvgServer03;
