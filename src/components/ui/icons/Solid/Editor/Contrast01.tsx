import * as React from "react";
import type { SVGProps } from "react";
const SvgContrast01 = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    role="img"
    {...props}
  >
    <path
      fill="currentColor"
      fillRule="evenodd"
      d="M13 3.055q.284.031.563.08a1 1 0 1 0 .344-1.97A11 11 0 0 0 12 1C5.925 1 1 5.925 1 12s4.925 11 11 11q.975 0 1.905-.165a1 1 0 0 0-.344-1.97 9 9 0 0 1-.561.08z"
      clipRule="evenodd"
    />
    <path
      fill="currentColor"
      d="M16.92 3.235a1 1 0 0 1 1.392-.245 11 11 0 0 1 2.7 2.701 1 1 0 0 1-1.637 1.149 9 9 0 0 0-2.212-2.212 1 1 0 0 1-.244-1.393M21.678 9.28a1 1 0 0 1 1.157.813 11 11 0 0 1 0 3.814 1 1 0 1 1-1.97-.345 9 9 0 0 0 0-3.124 1 1 0 0 1 .813-1.157M20.764 16.922a1 1 0 0 1 .244 1.393 11 11 0 0 1-2.697 2.695 1 1 0 1 1-1.148-1.637 9 9 0 0 0 2.208-2.207 1 1 0 0 1 1.393-.244"
    />
  </svg>
);
export default SvgContrast01;
