import * as React from "react";
import type { SVGProps } from "react";
const SvgCornerRightDown = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    role="img"
    {...props}
  >
    <path
      fill="currentColor"
      d="M6.4 5c1.697 0 2.909 0 3.86.078.938.077 1.533.224 2.01.467a5 5 0 0 1 2.185 2.185c.243.477.39 1.072.467 2.01.077.951.078 2.163.078 3.86v3.986l-3.293-3.293a1 1 0 0 0-1.414 1.414l5 5a1 1 0 0 0 1.414 0l5-5a1 1 0 0 0-1.414-1.414L17 17.586v-4.03c0-1.643 0-2.937-.085-3.978-.087-1.063-.267-1.95-.678-2.756a7 7 0 0 0-3.06-3.059c-.805-.41-1.692-.591-2.755-.678C9.381 3 8.087 3 6.444 3H3a1 1 0 0 0 0 2z"
    />
  </svg>
);
export default SvgCornerRightDown;
